...
A lower level may add detail inside the freedom left by higher levels. It may not weaken, broaden, contradict, or silently reinterpret higher intent. `state.json` records runtime facts only and has no authority over intent.
...
- Metadata inconsistency: state hashes, current operation, or evidence no longer match current artifacts/repository.
