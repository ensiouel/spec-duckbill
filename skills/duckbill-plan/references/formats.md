...
- Markdown checkboxes, execution status, attempts, evidence, and current operations are forbidden.
...
`Task N` is display numbering only. Kebab-case `ID` is stable identity. PRE, CHK, and VAL IDs are unique within tasks.md. Every VAL has a non-empty mapping and check text.

A task is one coherent implementation outcome, not one file edit or command. Context contains verified facts. Actions describe bounded work. Checks independently prove the result. Tasks never contain state.
